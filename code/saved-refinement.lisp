...
(defmacro process-conditionalization (&body body)
  `(flet ((internal (form) ,@body))
     (if (eql (first form) 'when)
	 (let ((answer (funcall #'internal (third form))))
	   `(when ,(devariablize (second form) 'raw) ,answer))
       (funcall #'internal form))))

(defun process-initial-input-description (form parent-name)
  (process-conditionalization
   (destructuring-bind (&key name type ((:port port-name)) ((:branch branch-name))) form
     (when (null port-name) (setq port-name name))
     `(add-initial-input ,parent-name ,(devariablize name) ,(devariablize port-name) 
