...
(defmethod consumes-input? ((task core-task-mixin)) t)
...
(defmethod consumes-input? ((task task-interface-mixin))
  (or (primitive? task)
      (dont-expand task)
...
(defmethod consumes-input? ((task initial-object-mixin)) nil)
;;; Final objects are exit-points, final-outputs and state-sinks
;;; the first two just pass the token up to the parent
(defmethod consumes-input? ((task final-object-mixin)) nil) 
;;; a State sink consumes the input
(defmethod consumes-input? ((task state-sink)) t)
...
(defmethod port-value-arrived ((p port) (direction (eql 'input)) (task core-task-mixin) value)
  ...
  (when (consumes-input? task) (push task (consumers value)))
...
(defmethod propagate-values-to-selected-implementation ((task input-side-mixin) implementation)
  (let* ((real-task (typecase task (join (superior task)) (otherwise task)))
	 ...
	 ;; the token along.
	 (introduce-internal-function  (getf (properties real-task) :introduce-labels)))
    (loop for input-port in (inputs task)
	for name = (name input-port)
	...
	if introduce-internal-function
	   ;; see comment above
	do (let ((symbolic-token (make-token top-level-argument
	   ...
	   ;; just push the token along unchanged
...
(defmethod detemporalize-token ((token t)) token)
(defmethod detemporalize-token ((token temporal-sequence-output-value)) (input-token token))
;;; this may be completely wrong
(defmethod detemporalize-token ((input-token stream-output-value)) (input-token token))
...
(defmethod propagate ((task task-interface) (type (eql 'take)))
  (let* ((input (first (inputs task)))
    ...
    ;; (or a dequeue form)
    (let ((new-token (detemporalize-token input-token)))
